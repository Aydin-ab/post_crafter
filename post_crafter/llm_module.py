...
class LLM:
    ...
    def generate_events_schedule(self, product_description, frequency_days, start_date= None, num_events= 5):
    ...
    # Create the next num_posts posts in the calendar (default to 10)
    ...
    def generate_posts_schedule(self, product_description, frequency_days, start_date= None, num_posts= 5, project_name=None):
        # If the start date is not provided, use today's date
        if not start_date:
            start_date = datetime.now().strftime('%m/%d/%Y')
        ...
        posts = []
        # Generate the next num_posts posts
        for i in range(num_posts):
            ...
            posts.append({"date": date_str, "caption": caption, "image": image_file})
        
        # Save the posts to a json file
        with open(f"{project_name}/schedule_posts.json", "w") as file:
